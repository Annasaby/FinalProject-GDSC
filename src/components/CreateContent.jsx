...
export default function CreatContent() {
  ...
  return (
    <div className="relative">
      <div onClick={()=>{setOpenform((prev) => !prev )}} className={`fixed ${openform ? 'rotate-45' : ''} top-40 right-5 bg-sky-400 p-2 w-fit rounded-full hover:scale-110 ease-in-out transition-all cursor-pointer z-30`}>
        <IoAdd className="text-white" />
      </div>

      <div className={`fixed ${openform ? '' : 'hidden'} top-0 z-20 bg-slate-600 opacity-50 h-screen w-screen`}></div>

      <div className={`fixed ${openform ? '' : 'hidden'} top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 px-5 py-10 w-3/4 bg-sky-500 rounded-xl`}>
